...
DeepSeek Harness Web 的**免 token 日历与天气插件**：在会话页头新增「熊猫日历」标签页（与「对话」「轨迹」「诗词」同级，排在诗词之后），一站式提供 **公历 / 农历 / 干支 / 生肖 / 节气 / 节日 / 中国法定节假日（含调休）/ 多城市天气 / ⏱ 时间戳转换 / 📜 历史上的今天**。
...
- **📜 历史上的今天**：今日卡内可折叠小节，按日期内联展示约 **3 条大事记 + 4~6 位精选名人诞辰**（历史名人为先）；**折叠态默认直接露出首条**（点击即可展开），右侧是**主色实心按钮**「展开全部 N 条 ▾ / 收起 ▴」（与时间戳卡的「现在」按钮同款形制：实心填充 + 8px 圆角 + 投影 + 13px/600 字重）；内置 **366 天离线快照**（含 2/29，大陆裸网环境也完整可用），网络可达中文维基时自动增强当日最新内容（12h 缓存），来源标注 CC BY-SA
...
- **⏱ 时间戳转换**（天气卡下方独立卡片，纯前端计算、离线可用）：**时间戳 → 日期**（10 位秒 / 13 位毫秒自动识别，也可手动指定）与 **日期 → 时间戳**（年月日时分秒 → 秒级 / 毫秒级）双向互转；右上角**时区选择**（本地 / UTC / 京沪港台东京新加坡新德里迪拜莫斯科伦敦巴黎纽约芝加哥洛杉矶悉尼，经 `Intl` 取偏移、**自动处理夏令时**）；结果含星期、UTC 偏移、ISO 8601（UTC）与一键复制
...
2. 今日卡默认显示今天：农历、干支、生肖、节气、节日、法定状态一目了然；「📜 历史上的今天」折叠时已显示首条，点首条或右侧「展开全部 N 条 ▾」查看当日全部大事记与名人诞辰（「收起 ▴」收回）。
3. 点 **✍ 发送到对话**，该日摘要写入输入框（**不自动发送**），切回对话视图按回车即可让 AI 解读 / 安排。
4. 月历支持翻月 / 「今天」/ 点选日期查看任意一天；天气卡可搜索添加任意城市、点击城市切换。
5. **⏱ 时间戳转换**（天气卡下方）：切「时间戳 → 日期」直接粘入 10 位（秒）或 13 位（毫秒）数字即时得到年月日时分秒；切「日期 → 时间戳」填写年月日时分秒得到秒级 / 毫秒级时间戳；两者共用右上角**时区选择**（本地 / UTC / 京沪港台东京新加坡新德里迪拜莫斯科伦敦巴黎纽约芝加哥洛杉矶悉尼，自动处理夏令时），结果带 ISO 8601（UTC）与一键复制。
...
单测覆盖：历法引擎（911 组逐日 fixtures + 闰月 / 春节换年 / 节气口径 / 双向换算 / 越界）、节日规则（含复活节与农历年边界）、法定数据规整与徽标、天气解析与城市搜索、历史上的今天（快照查找 / 在线拉取缓存与静默失败 / 数据无 HTML 残留且与 fixture 无漂移）、时间戳转换（秒毫秒识别与边界、多时区格式化与反解、夏令时前后跳变、非法输入）、视图模型（月历网格 / 今日卡 / 发送文本 / history 小节 / 时间戳卡片位置）与注册冒烟。
